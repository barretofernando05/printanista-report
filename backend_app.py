...
app = FastAPI(title="Printanista Proyecto Corregido", version="7.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def exec_sql(sql: str, params: dict[str, Any] | None = None):
    with engine.begin() as conn:
        conn.execute(text(sql), params or {})

def safe_rows(sql: str, params: dict[str, Any] | None = None):
    try:
        return rows(sql, params)
    except Exception:
        return []

def safe_one(sql: str, params: dict[str, Any] | None = None):
    try:
...
def dashboard_summary(date_from: str | None = None, date_to: str | None = None):
...
def detail_alertas(cliente: str, date_from: str | None = None, date_to: str | None = None):
    where, params = build_filters(date_from, date_to)
...
def finish_job(job_id: int, status: str, rows_inserted: int = 0, rows_updated: int = 0, rows_ignored: int = 0, error_text: str | None = None, details: str | None = None):
    exec_sql("""
        UPDATE job_runs
        SET status=:status,
            finished_at=NOW(),
...
async def _generic_import(file: UploadFile, job_name: str, target_table: str):
    ...
    try:
        ...
        finish_job(job_id, "success", rows_inserted=1, details=f"sha1={file_sha1}")
        return {
            "status": "ok",
            "job_id": job_id,
