...
export default function ReemplazosPage({ openSerie }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const result = await fetchJson(`/api/operaciones/reemplazos?${qs}`);
      setData(result);
    } catch (error) {
      console.error(error);
  ...
  const exportExcel = () => {
    const qs = buildQueryString();
    const url = qs
      ? `/api/operaciones/reemplazos/export?${qs}`
      : `/api/operaciones/reemplazos/export`;
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: stacked ? "1fr 1fr" : "repeat(4, minmax(0, 1fr))",
            gap: 16,
            marginBottom: 16,
          }}
        >
