...
export default function App() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setDashboard({
        ...
        modelos: summary.modelos || [],
        timeline: summary.timeline || [],
        reemplazos_mes: summary.reemplazos_mes || [],
        equipos_modelo: summary.equipos_modelo || [],
      });
      setJobs(jobsData || []);
    } catch (e) {
      setError(e.message);
  ...
  async function uploadFile(kind, file) {
    ...
    try {
      ...
      fd.append("file", file);
      const data = await fetchJson(`/api/import/${kind}`, {
        method: "POST",
        body: fd,
      });
  ...
  async function loadClientDetail(cliente) {
    try {
      const url = `/api/detail/alertas?cliente=${encodeURIComponent(cliente)}${query ? "&" + query.slice(1) : ""}`;
    ...
    <div style={styles.shell}>
      <aside style={styles.aside}>
        <div style={{ marginBottom: 28 }}>
          <div style={{ color: "#dc2626", fontWeight: 900, fontSize: 42, letterSpacing: -1.6 }}>
            RICOH
          </div>
          <div style={{ fontSize: 22, fontWeight: 700, marginTop: 6 }}>TECHNOMA</div>
        </div>

        <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 14, letterSpacing: 0.8 }}>
          PRINTANISTA DB OPS
        </div>

        <button style={styles.nav(view === "dashboard")} onClick={() => setView("dashboard")}>
          <LayoutDashboard size={18} />
          Dashboard
        </button>
        <button style={styles.nav(view === "import")} onClick={() => setView("import")}>
          <UploadCloud size={18} />
          Importar / Sync
        </button>
        <button style={styles.nav(view === "history")} onClick={() => setView("history")}>
          <History size={18} />
          Historial
        </button>
        <button style={styles.nav(view === "busqueda")} onClick={() => setView("busqueda")}>
          <Search size={18} />
          Consultar Equipo
        </button>
      </aside>
      ...
      <main style={styles.main}>
        ...
        >
          ...
          <button style={styles.ghost} onClick={loadDashboard}>
            <RefreshCw size={16} style={{ verticalAlign: "middle", marginRight: 8 }} />
            Refrescar
          </button>
        </div>
            ...
            <section style={{ ...styles.grid, marginBottom: 18 }}>
              <div style={styles.card}>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, marginTop: 8 }}>
                  {fmt(dashboard.kpis.equipos_monitoreados)}
                </div>
              </div>
              ...
              <div style={styles.card}>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, marginTop: 8 }}>
                  {fmt(dashboard.kpis.alertas_activas)}
                </div>
              </div>
              ...
              <div style={styles.card}>
                ...
                <div style={{ fontSize: 30, fontWeight: 800, marginTop: 8 }}>
                  {fmt(dashboard.kpis.porc_equipos_con_alertas)}%
                </div>
              </div>
            </section>
              ...
              <div style={styles.card}>
                ...
                <div style={{ width: "100%", height: 320 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={dashboard.clientes || []}
                      onClick={(e) => {
                        if (e?.activeLabel) loadClientDetail(e.activeLabel);
                      }}
                    >
                ...
                <div style={{ color: "#64748b", fontSize: 12 }}>
                  Haz click en una barra para abrir el detalle.
                </div>
              </div>
            ...
            >
              <div style={styles.card}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <button style={styles.btn} onClick={() => runSync("bd3")}>
                    <MailCheck size={16} style={{ marginRight: 8, verticalAlign: "middle" }} />
                    Sync BD3
                  </button>
                  <button style={styles.btn} onClick={() => runSync("bd4")}>
                    <MailCheck size={16} style={{ marginRight: 8, verticalAlign: "middle" }} />
                    Sync BD4
                  </button>
                  <button style={styles.btn} onClick={() => runSync("all")}>
                    <MailCheck size={16} style={{ marginRight: 8, verticalAlign: "middle" }} />
                    Sync All
                  </button>
                </div>
                <div style={{ color: "#64748b", fontSize: 12, marginTop: 10 }}>
                  Requiere <code>secrets/token_technoma.json</code> montado en <code>/app/secrets</code>.
                </div>
              </div>
        ...
        {view === "busqueda" && (
          <section style={styles.card}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              />
              <button style={styles.btn} onClick={buscarEquipo}>
                Buscar
              </button>
            </div>
          </section>
