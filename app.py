...
app = FastAPI(title="Printanista Report V3", version="3.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def get_headers(full: dict) -> dict:
    headers = {}
...
def load_bd1_manual(file_bytes: bytes, sourcefile: str, sheet_name: str = "Reporte") -> dict[str, Any]:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text("INSERT IGNORE INTO printanista.processed_files_bd1 (file_sha1, sourcefile, processed_ts) VALUES (:s,:f,NOW())"),
...
def load_bd3_manual(file_bytes: bytes, sourcefile: str) -> dict[str, Any]:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text("INSERT IGNORE INTO printanista_insumos.processed_files_bd3 (file_sha1, sourcefile, processed_ts) VALUES (:s,:f,NOW())"),
            {"s": file_sha1, "f": sourcefile},
        )
...
def parse_yymmdd_from_filename(filename: str, pattern: str) -> str | None:
    m = re.search(pattern, filename, flags=re.IGNORECASE)
    return m.group(1) if m else None

def read_xlsx_first_sheet_from_bytes(data: bytes) -> pd.DataFrame:
...
def sync_bd2_alertas(max_results: int = 100) -> dict[str, Any]:
    ...
    for m in msgs:
        ...
        for filename, attachment_id in attachments:
            if not re.match(r"^TECHNOMA_Alertas_Active Alerts_(\d{6})\.xlsx$", filename, flags=re.IGNORECASE):
                continue
            ...
            df = read_xlsx_first_sheet_from_bytes(data)
            payload_cols = [c for c in df.columns]
            inserted_here = 0
            with engine.begin() as conn:
                for _, row in df.iterrows():
                    payload = {str(k): (None if pd.isna(v) else str(v)) for k, v in row.to_dict().items()}
                    numero_serie_txt = payload.get("Número Serie") or payload.get("Numero Serie") or payload.get("numero_serie")
                    row_hash = hashlib.sha1(json_dumps(payload).encode("utf-8")).hexdigest()
                    conn.execute(text("""
                        INSERT IGNORE INTO printanista_alertas.alertas_actives
                        (row_hash, load_ts, report_date, sourcefile, sheet_name, alerta_json, numero_serie_txt)
                        VALUES (:row_hash, NOW(), :report_date, :sourcefile, 'sheet1', :alerta_json, :numero_serie_txt)
                    """), {
                        "row_hash": row_hash,
                        "report_date": report_date,
                        "sourcefile": filename,
                        "alerta_json": json_dumps(payload),
                        "numero_serie_txt": numero_serie_txt,
                    })
                    inserted_here += 1
                conn.execute(text("""
...
def get_table_cols(schema: str, table: str) -> list[str]:
...
def sync_bd3_insumos(max_results: int = 100) -> dict[str, Any]:
    ...
    msgs = gmail_search(service, query=query, max_results=max_results)
    data_cols = get_table_cols("printanista_insumos", "dispositivos_detallado_gv2")
    ...
    for m in msgs:
        ...
        for filename, attachment_id in attachments:
            ...
            df = read_xlsx_first_sheet_from_bytes(data)
            df.columns = [make_unique_cols_name(sanitize_col(c), idx) for idx, c in enumerate(df.columns)]
            ...
            if "numero_serie_idx" in data_cols:
                if "numero_serie" in df.columns:
                    df["numero_serie_idx"] = df["numero_serie"].apply(lambda v: "" if is_missing(v) else str(v).strip())
                else:
                    df["numero_serie_idx"] = ""

            for c in data_cols:
                if c not in df.columns:
                    df[c] = None if c in ("report_date", "report_date_idx") else ""

            df = df[data_cols].copy()
            insert_cols = ["row_hash", "load_ts"] + data_cols
            cols_sql = ", ".join(f"`{c}`" for c in insert_cols)
            values_sql = ", ".join(f":{c}" for c in insert_cols)
            upd_sql = ", ".join([f"`{c}`=VALUES(`{c}`)" for c in insert_cols if c != "row_hash"])
            ...
            with engine.begin() as conn:
                for _, row in df.iterrows():
                    values = []
                    rec = {"load_ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
                    for c in data_cols:
                        v = row[c]
                        if isinstance(v, date):
                            ...
                            rec[c] = "" if is_missing(v) else str(v)
                            values.append(rec[c])
...
def make_unique_cols_name(name: str, idx: int) -> str:
    return name

def sync_bd4_reemplazos(max_results: int = 100) -> dict[str, Any]:
    service = gmail_service()
    query = 'from:no-reply@printanistahub.com subject:"Reporte Programado v4" filename:xlsx newer_than:60d'
    msgs = gmail_search(service, query=query, max_results=max_results)
    data_cols = get_table_cols("printanista_reemplazos", "reemplazos_insumos_gv")
    processed = skipped = rows_attempted = 0
    ...
    for m in msgs:
        ...
        for filename, attachment_id in attachments:
            ...
            cols_sql = ", ".join(f"`{c}`" for c in insert_cols)
            values_sql = ", ".join(f":{c}" for c in insert_cols)
            sql = f"INSERT IGNORE INTO printanista_reemplazos.reemplazos_insumos_gv ({cols_sql}) VALUES ({values_sql})"
            ...
            with engine.begin() as conn:
                for _, row in df.iterrows():
                    ...
                    for c in data_cols:
                        v = row[c]
                        rec[c] = "" if is_missing(v) else v
                        vals.append(rec[c])
                    rec["row_hash"] = row_hash_from_values(vals)
                    conn.execute(text(sql), rec)
                    rows_attempted += 1
                conn.execute(text("""
...
def reports_overview():
    ...
    if "alertas_total" in resumen_cols:
        equipos_con_alertas = one("SELECT COUNT(*) AS total FROM printanista_insumos.vw_equipo_insumos_con_alertas WHERE alertas_total > 0")["total"]
    return {
        "equipos_con_alertas": equipos_con_alertas,
        "alertas_activas": one("SELECT COUNT(*) AS total FROM printanista_alertas.vw_alertas_actives")["total"],
        "reemplazos": one("SELECT COUNT(*) AS total FROM printanista_reemplazos.vw_reemplazos_insumos_pct")["total"],
        "equipos_resumen": one("SELECT COUNT(*) AS total FROM printanista_insumos.vw_equipo_insumos_resumen")["total"],
    }
